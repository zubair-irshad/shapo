...
def get_surface_pointclouds(latent_vector, sdf_pretrained_dir):
    specs_filename = os.path.join(sdf_pretrained_dir, "specs.json")
    ...
    saved_model_state = torch.load(
        os.path.join(sdf_pretrained_dir, model_params_subdir, checkpoint + ".pth")
    )
...
def get_surface_pointclouds_octgrid(latent_vector, octgrid, lods, decoder):
    ...
    # latent_vector = torch.from_numpy(latent_vector).cuda()
    # latent_vector = torch.from_numpy(latent_vector).cuda()
    # print("latent_vector device", latent_vector.device)
    # latent_vector = torch.from_numpy(latent_vector).cuda()
    latent_vector = torch.from_numpy(latent_vector).cuda()
    latent_vector = latent_vector.squeeze(-1)
    latent_vector = latent_vector.float()
    ...
    pred_pcd = octgrid.get_surface_points_given_hook(pred_sdf_grid, grid_3d, threshold=get_cell_size(lods[-1]))
    del grid_3d
    return pred_pcd


def get_surface_pointclouds_octgrid_sparse(latent_vector):
    specs_filename = "/home/ubuntu/generalizable-object-representations/sdf_latent_codes/all_ws_no_reg_contrastive0.1/specs.json"
    model_params_subdir = "ModelParameters"
    checkpoint = "2000"
    sdf_latent_code_dir = "/home/ubuntu/generalizable-object-representations/sdf_latent_codes/all_ws_no_reg_contrastive0.1"
    ...
    # inputs_sdfnet = torch.cat([latent_vector.expand(grid_3d.shape[0], -1), grid_3d], 1).to(latent_vector.device)
    # pred_sdf_grid = decoder(inputs_sdfnet)
    # pred_pcd = octgrid.get_surface_points_sparse(pred_sdf_grid, grid_3d, threshold=get_cell_size(lods[-1]))
    # return pred_pcd.detach().cpu().numpy()
    return grid_3d.cpu().numpy()

def get_surface_pointclouds_sparse(latent_vector):
    specs_filename = "/home/ubuntu/generalizable-object-representations/sdf_latent_codes/all_ws_no_reg_contrastive0.1/specs.json"
    model_params_subdir = "ModelParameters"
    checkpoint = "2000"
    sdf_latent_code_dir = "/home/ubuntu/generalizable-object-representations/sdf_latent_codes/all_ws_no_reg_contrastive0.1"
    ...
    # Create 3D grid and form DeepSDF input
    grid_density = 30  # dummy grid density
    device = torch.device('cuda')
    grid_3d = Grid3D(grid_density, device)
    latent_repeat = latent_vector.expand(grid_3d.points.size(0), -1)
    inputs = torch.cat([latent_repeat, grid_3d.points], 1).to(latent_vector.device, latent_vector.dtype)
    # with torch.no_grad():
    pred_sdf_grid = decoder(inputs)
    pcd_dsdf = grid_3d.get_masked_surface_points(pred_sdf_grid)
    return pcd_dsdf.detach().cpu().numpy()

def get_surface_pointclouds_octgrid_viz(latent_vector, lod, sdf_pretrained_dir):
    specs_filename = os.path.join(sdf_pretrained_dir, "specs.json")
    model_params_subdir = "ModelParameters"
    checkpoint = "2000"
    specs = json.load(open(specs_filename))
...
def get_grid_sdfnet_octgrid(latent_vector):
    specs_filename = "/home/ubuntu/generalizable-object-representations/sdf_latent_codes/all_ws_no_reg_contrastive0.1/specs.json"
    model_params_subdir = "ModelParameters"
    checkpoint = "2000"
    sdf_latent_code_dir = "/home/ubuntu/generalizable-object-representations/sdf_latent_codes/all_ws_no_reg_contrastive0.1"
