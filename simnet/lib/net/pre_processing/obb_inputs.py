...
def compute_nocs_network_targets(masks, shape_code, appearancecode, poses, height, width, emb_dim = 64):
  if len(masks) == 0:
    ...
    return datapoint.ABSPOSE(
        ...
        shape_emb=np.zeros([height_d, width_d, emb_dim]),
        appearance_emb = np.zeros([height_d, width_d, emb_dim]),
        abs_pose = np.zeros([height_d, width_d, 13])
    )
  heatmaps = compute_heatmaps_from_masks(masks)
  shape_emb_target = compute_latent_emb(masks, shape_code, heatmaps, emb_dim)
  appearance_emb_target = compute_latent_emb(masks, appearancecode, heatmaps, emb_dim)
  abs_pose_target = compute_nocs_abspose_field(poses, heatmaps)
  
  return datapoint.ABSPOSE(
      heat_map=np.max(heatmaps, axis=0),
      shape_emb= shape_emb_target,
      appearance_emb = appearance_emb_target,
...
def compute_latent_emb(obbs, point_embs, heat_maps, emb_dim):
  latent_emb_target = np.zeros([len(obbs), heat_maps[0].shape[0], heat_maps[0].shape[1], emb_dim])
  heatmap_indices = np.argmax(np.array(heat_maps), axis=0)
